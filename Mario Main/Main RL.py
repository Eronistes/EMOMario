...
''' Resize 84x84 
     def __build_cnn(self, c, output_dim):
        return nn.Sequential(
            nn.Conv2d(in_channels=c, out_channels=32, kernel_size=8, stride=4),  # Output: (32, 14, 14)
            nn.ReLU(),
            nn.Conv2d(in_channels=32, out_channels=64, kernel_size=4, stride=2),  # Output: (64, 6, 6)
            nn.ReLU(),
            nn.Conv2d(in_channels=64, out_channels=64, kernel_size=3, stride=1),  # Output: (64, 4, 4)
            nn.ReLU(),
            nn.Conv2d(in_channels=64, out_channels=64, kernel_size=3, stride=2),  # Output: (64, 1, 1)
            nn.ReLU(),
            nn.Flatten(),  # Output: 64 * 1 * 1
            nn.Linear(64 * 1 * 1, 512),  # Ensure this matches the final conv layer output
            nn.ReLU(),
            nn.Linear(512, output_dim),
        )
'''
...
'''  Resize 120x120
    def __build_cnn(self, c, output_dim):
        return nn.Sequential(
            ...
            nn.ReLU(),
            nn.Flatten(),
            nn.Linear(64 * 5 * 5, 512),  # Adjusted linear layer input size
            nn.ReLU(),
            nn.Linear(512, output_dim),
    )
...
class Mario(Mario):
    def save(self, current_step):
        save_path = self.save_dir / f"mario_net_step:{int(current_step)}.pth"
        torch.save(
            {
            "online_state_dict": self.net.online.state_dict(),
            "target_state_dict": self.net.target.state_dict(),
            "exploration_rate": self.exploration_rate,
            },
            save_path,
        )
        print(f"MarioNet saved to {save_path} at step {current_step}")

    def load(self, load_path):
        checkpoint = torch.load(load_path)
        self.net.online.load_state_dict(checkpoint["online_state_dict"])
        self.net.target.load_state_dict(checkpoint["target_state_dict"])
